...
async function goTo(bot, block, reach = 2) {
    const { x, y, z } = block.position;
    return new Promise((resolve, reject) => {
        bot.pathfinder.setGoal(new mineflayer_pathfinder_1.goals.GoalNear(x, y, z, reach), true);
...
async function findOrPlaceFurnace(bot) {
    const mcData = require('minecraft-data')(bot.version);
...
// ─── Smelting ───────────────────────────────────────────────────────────────
async function executeSmelt(bot, recipe, quantity) {
    const furnaceBlock = await findOrPlaceFurnace(bot);
    if (!furnaceBlock)
        return { success: false, reason: 'cannot find or place furnace' };
    try {
        ...
        while (Date.now() - start < maxWait) {
            await sleep(1_000);
            if ((furnace.outputItem()?.count ?? 0) >= quantity)
                break;
        }
 ...
 */
async function ensureItem(bot, item, quantity, depth = 0) {
    if (depth > 12)
        return { success: false, reason: `recursion too deep trying to get ${item}` };
    // Already have enough?
    ...
    // Try crafting recipe
    const craftRecipe = (0, recipes_1.getCraftingRecipe)(item);
    if (craftRecipe) {
        ...
        for (const [ingredient, perRun] of Object.entries(craftRecipe.requires)) {
            ...
            if (!sub.success)
                return { success: false, reason: `cannot get ingredient ${ingredient}: ${sub.reason}` };
        }
        // Now craft
        return executeCraftSingle(bot, item, runs);
    }
    // Try smelting recipe
    const smeltRecipes = (0, recipes_1.getSmeltingRecipes)(item);
    if (smeltRecipes.length > 0) {
        ...
        if (!inputSub.success)
            return { success: false, reason: `cannot get ${smelt.input}: ${inputSub.reason}` };
        // Ensure fuel
        const fuelNeeded = Math.ceil(smelt.fuelPerSmelt * still);
        const fuelSub = await ensureItem(bot, smelt.fuel, fuelNeeded, depth + 1);
        if (!fuelSub.success)
            return { success: false, reason: `cannot get fuel ${smelt.fuel}: ${fuelSub.reason}` };
...
async function executeCraftSingle(bot, target, runs = 1) {
    const mcData = require('minecraft-data')(bot.version);
    const targetId = mcData.itemsByName[target]?.id;
    if (!targetId)
        return { success: false, reason: `unknown item: ${target}` };
    const craftRecipe = (0, recipes_1.getCraftingRecipe)(target);
    if (!craftRecipe)
        return { success: false, reason: `no crafting recipe for ${target}` };
    let ctBlock = null;
    if (craftRecipe.station === 'crafting_table') {
        ctBlock = await findOrPlaceCraftingTable(bot);
        if (!ctBlock)
            return { success: false, reason: 'cannot find or place crafting table' };
    }
    const recipe = bot.recipesFor(targetId, null, 1, (ctBlock ?? null))[0];
    if (!recipe)
        return { success: false, reason: `mineflayer found no recipe for ${target} — missing materials?` };
    try {
        await bot.craft(recipe, runs, (ctBlock ?? null));
...
async function executeCraft(bot, target, quantity = 1) {
    ...
    const inv = Object.fromEntries(bot.inventory.items().map(i => [
        Object.values(mcData.items)
            .find((d) => d.id === i.type)?.name ?? String(i.type),
        i.count,
    ]));
    const check = (0, recipes_1.checkCraftability)(target, quantity, inv);
    if (!check.canCraft) {
