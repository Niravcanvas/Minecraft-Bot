...
function isUnderThreat(bot, radius = 12) {
...
function hasPickaxeTier(bot, tier) {
    const items = bot.inventory.items().map(i => i.name);
    if (tier === 'any')
        return items.some(n => n.includes('pickaxe'));
    if (tier === 'stone')
        return items.some(n => n === 'stone_pickaxe' || hasPickaxeTier(bot, 'iron'));
    if (tier === 'iron')
        return items.some(n => n === 'iron_pickaxe' || hasPickaxeTier(bot, 'diamond'));
    if (tier === 'diamond')
        return items.some(n => n === 'diamond_pickaxe');
    return false;
}
...
function hasLogs(bot) {
    return bot.inventory.items().some(i => blocks_1.WOOD_BLOCKS.some(w => i.name === w));
}
function hasPlanks(bot) {
    return bot.inventory.items().some(i => i.name.includes('_planks'));
}
// ─── Wood count helper ────────────────────────────────────────────────────────
function logCount(bot) {
    return bot.inventory.items()
        .filter(i => blocks_1.WOOD_BLOCKS.some(w => i.name === w))
        .reduce((s, i) => s + i.count, 0);
}
function detectPhase(bot) {
    if (hasPickaxeTier(bot, 'iron') && hasFullIronArmor(bot))
        return 'late_game';
    if (hasPickaxeTier(bot, 'stone') && inv(bot, 'furnace'))
        return 'mid_game';
    return 'early_game';
}
...
class Brain {
    ...
    failHistory = [];
    suppressed = new Map(); // key → suppress-until ts
    ...
    recordOutcome(goal, target, success, reason) {
        ...
        if (success) {
            ...
            if (goal === 'survive' && target === 'flee' && reason.includes('no hostiles')) {
                ...
                logger_1.log.brain(`[flee] safe cooldown for ${FLEE_SAFE_COOLDOWN_MS / 1000}s`);
            }
        ...
        const recentFails = this.failHistory.filter(r => r.goal === goal && r.target === target).length;
        if (recentFails >= LOOP_THRESHOLD) {
            const until = now + SUPPRESS_MS;
            this.suppressed.set(key, until);
            logger_1.log.warn(`[brain] suppressing ${key} for ${SUPPRESS_MS / 1000}s (failed ${recentFails}x)`);
    ...
    async pickGoal() {
        ...
        if (det) {
            if (this.isSuppressed(det.goal, det.target)) {
                logger_1.log.brain(`[det] suppressed ${det.goal}(${det.target}), skipping`);
            }
            else {
        ...
        while (this.strategyQueue.length > 0) {
            ...
            if (this.alreadyAchieved(next)) {
                logger_1.log.brain(`[strategy] skip ${next.goal}(${next.target}) — already done`);
                this.strategyQueue.shift();
                continue;
            }
            if (this.isSuppressed(next.goal, next.target)) {
                logger_1.log.brain(`[strategy] suppressed ${next.goal}(${next.target}), skipping`);
                this.strategyQueue.shift();
                continue;
            }
            this.strategyQueue.shift();
        ...
        if (now - this.lastLLMCall > 60_000) {
            ...
            try {
                ...
                logger_1.log.brain(`[llm] suppressed ${goal.goal}(${goal.target}), using fallback`);
            }
            catch (err) {
                logger_1.log.warn(`LLM failed: ${err.message} — using fallback`);
            }
        }
    ...
    async llmGoal() {
        ...
        const suppressedList = [...this.suppressed.entries()]
            ...
            .map(([key]) => key)
            .join(', ');
        const prompt = `phase=${phase} hp=${Math.round(this.bot.health)}/20 food=${Math.round(this.bot.food)}/20 ` +
