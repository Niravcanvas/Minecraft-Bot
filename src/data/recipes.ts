...
export type CraftingStation = 'hand' | 'crafting_table' | 'furnace';

export interface CraftingRecipe {
  kind: 'crafting';
  result: string;
  count: number;                   // how many you get per craft
...
export interface SmeltingRecipe {
  ...
  result: string;
  count: number;
  input: string;                   // what goes IN the furnace
  fuel: string;                    // e.g. 'log', 'coal', 'planks'
  fuelPerSmelt: number;            // how much fuel per operation
}

export type Recipe = CraftingRecipe | SmeltingRecipe;
...
export function getCraftingRecipe(result: string): CraftingRecipe | null {
  return CRAFTING_RECIPES.find(r => r.result === result) ?? null;
}

/** Find all smelting recipes that produce a given result. */
export function getSmeltingRecipes(result: string): SmeltingRecipe[] {
  return SMELTING_RECIPES.filter(r => r.result === result);
}

/** Find any recipe (crafting OR smelting) that produces a result. */
export function getAnyRecipe(result: string): Recipe | null {
...
export interface RawMaterials {
  /** Items the bot must gather from the world (ore, logs, cobblestone…) */
  gather: Record<string, number>;
  /** Items that must be smelted in a furnace before crafting */
  smelt: Record<string, number>;
  /** Intermediate crafted items (the bot makes these itself along the way) */
 ...
 */
export function resolveRawMaterials(
  item: string,
  quantity: number,
  visited: Set<string> = new Set(),
): RawMaterials {
  const result: RawMaterials = { gather: {}, smelt: {}, craft: {} };
...
function mergeInto(target: RawMaterials, source: RawMaterials): void {
  for (const [k, v] of Object.entries(source.gather)) target.gather[k] = (target.gather[k] ?? 0) + v;
  for (const [k, v] of Object.entries(source.smelt))  target.smelt[k]  = (target.smelt[k]  ?? 0) + v;
  for (const [k, v] of Object.entries(source.craft))  target.craft[k]  = (target.craft[k]  ?? 0) + v;
}
...
export interface CraftabilityResult {
  canCraft: boolean;
  missing: Record<string, number>;   // what the bot still needs to gather
 ...
 */
export function checkCraftability(
  item: string,
  quantity: number,
  ...
  const needsCraftingTable = (() => {
    ...
    for (const craftedItem of Object.keys(raw.craft)) {
      ...
      if (r && r.station === 'crafting_table') return true;
    }
    // Also check the top-level item
    const topRecipe = getCraftingRecipe(item);
    if (topRecipe && topRecipe.station === 'crafting_table') return true;
    return false;
  })();
 ...
 */
export function nextProgressionGoal(crafted: Set<string>): string | null {
  return PROGRESSION.find(item => !crafted.has(item)) ?? null;
}