...
export async function executeSocial(bot: Bot, target: string, trust: TrustMemory): Promise<{ success: boolean; reason: string }> {
  ...
  if (target === 'follow_trusted') {
    ...
    if (!trusted.length) return { success: false, reason: 'no trusted players online' };
    const t = trusted[0];
